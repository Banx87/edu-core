                ...
                <div class="col-xl-9 col-lg-8 order-lg-1">
                    ...
                    <div class="row">
                        @forelse ($courses as $course)
                            <x-course-card :thumbnail="$course->thumbnail" :title="$course->title" :duration="$course->duration" :url="route('courses.show', $course->slug)"
                                :instructor="$course->instructor" :price="$course->price" :discount="$course->discount" :lessons="$course->lessons()->count()" :students="$course->enrollments->count()"
                                :rating="$course->reviews()->avg('rating')" :id="$course->id" />
                        @empty
                            <p>No data found</p>
                        @endforelse
                    </div>
